...
/**
 * @Auther: zhonglin
 * 根据 建表SQL语句生成mybatis ResultMap语句！
...
public class MybatisResultUtil {
    public static void main(String[] args) throws IOException {
        ...
        MybatisResultUtil main = new MybatisResultUtil();
        main.getModel(filePath);
        main.getResultMapXML(filePath);
        main.getInsert(filePath);
    }//main
     ...
     * @param filePath
     */
    private void getModel(String filePath) throws FileNotFoundException {
...
private String getInsert(String filePath) throws IOException {
    InputStream is = new FileInputStream(filePath);
    BufferedReader reader = new BufferedReader(new InputStreamReader(is, "utf-8"), 512);
    // 读取一行，存储于字符串列表中
    System.out.println();
    System.out.println("<!--  ----------以下是生成的INSERT代码---------  -->");
    String tableName = null;
    String typeName = null;
    String propertyName;
    String columnName = null;

    String insertIfValues = "";
    String insertIfValue = "";
    String insertPre = "";
    for (String line = reader.readLine(); line != null; line = reader.readLine()) {
        line = line.trim();
        if (line == null || line.length() == 0 || line.contains("-") || line.contains("DROP")) {
        ...
        if (line.contains("CREATE")) {
            tableName = line.substring(line.indexOf("`") + 1, line.lastIndexOf("`"));
            tableName = this.getTableName(tableName);
            insertPre = MessageFormat.format("<insert id=\"insert{0}\" parameterType=\"{1}\">\n" +
                    "        insert into {2} \n   <trim prefix=\"(\" suffix=\")\" suffixOverrides=\",\">\n", tableName,tableName,line.substring(line.indexOf("`") + 1, line.lastIndexOf("`")));
        }
        propertyName = this.getPropertyName(line);
        if (!"NULL".equals(propertyName)) {
            columnName = this.getColumnName(line);
            typeName = this.getType(line);
            String ifNoll = typeName.equals("String")?String.format(" and %s!=''",propertyName):"";
            insertIfValues = insertIfValues + String.format("\t<if test=\"%s != null"+ ifNoll +"\">\n" +
                    "     %s,\n" +
